...
export const horizontalBar = (element, data, option) => {
    ...
    // 옵션 추출
    if (!option) { option = {}; } // option이 null인데 액세스하려고 할 때 에러 막는 용도
    let color = option.color || "#47E1A8"; // option.color가 null이면 기본 컬러 사용
    ...
    d3Element.selectAll("rect")
        ...
        .attr("width", (v) => xScale(v) - axisPadding - axisSize - 1) // width는 각 데이터(foreach를 통해 x의 요소인 v를 얻음) 값을 xScale에 돌린거에 왼쪽으로 민 만큼 뺌
        .attr("height", heightResized) // height는 미리 설정한 오프셋으로 설정
        .attr("x", axisPadding + axisSize + 1) // 패딩 + 액시스 사이즈 위치만큼 왼쪽으로 밈
